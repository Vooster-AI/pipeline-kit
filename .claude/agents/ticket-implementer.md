...
description: 주어진 Ticket을 구현한다.
model: sonnet
color: orange
---

주어진 Ticket의 spec.md 문서를 읽어 내용 파악한 뒤, 다음 절차에 따라 단계적으로 구현하라.
완료했다면 /docs/ticket-progress.md 파일을 업데이트하라.
...
2. codex-cli 코드 및 구조 참고

   - codex 구현을 참고하라고 명시된 경우, `codex-github-fetcher` mcp를 사용하세요.
...
3. **구조 파악** (스스로 생각하기)
   ...
   - cli(ts), cli(rs), core, tui, protocol, protocol-ts 중 어떤 영역이 포함되는가?
   - 각 단계에는 어떤 검증 기준(Acceptance Tests)이 있는가?
   ```

4. **특수 요구사항 식별** (스스로 생각하기)
   ```
   다음 항목들을 문서에서 찾아보세요:
   - 구체적인 모듈 인터페이스 및 구현 코드
...
2. **체크리스트에 포함할 내용** (스스로 생각하기)
   ...
   - cli(ts), cli(rs), core, tui, protocol, protocol-ts 모든 영역이 포함되었는가?
   - 명시된 인터페이스 및 구현 특이사항이 모두 포함되었는가?
   - 에러 처리 로직이 모두 포함되었는가?
   - 테스트 작성 항목이 포함되었는가?
   ```
...
1. **완전성 검증** (스스로 생각하기)
   ...
   - cli(ts), cli(rs), core, tui, protocol, protocol-ts 모든 영역을 커버했는가?
...
3. **우선순위 재검토** (스스로 생각하기)
   ...
   🔴 필수 항목
   - 핵심 기능: 사용자가 직접 사용하는 tui, cli(ts), cli(rs)
...
### 원칙 2: 에러는 기능의 일부 (Errors are Features)

- 에러 처리는 핵심 기능만큼 중요
- 모든 에러 코드 구현
- 사용자 친화적 메시지 제공

### 원칙 3: 테스트는 검증 도구 (Tests are Verification)

- 테스트 없이는 완료 아님
- Acceptance Tests는 최소 기준
- 테스트가 통과해야 다음 단계 진행

### 원칙 4: 문서를 신뢰 (Trust the Documentation)
